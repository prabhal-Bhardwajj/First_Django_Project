...
def journal_list(request):
    journals = Journal.objects.all().order_by('-id')  # Fetch journals sorted by ID
    return render(request, 'Journalapp/journal_list.html', {'journals': journals})
...
def add_journal(request):
    ...
    Handles form submission and displays the form for new entries.
    """
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if form.cleaned_data['long_term_goal']:
                ...
                # You can add additional fields to LongTermGoal as needed
